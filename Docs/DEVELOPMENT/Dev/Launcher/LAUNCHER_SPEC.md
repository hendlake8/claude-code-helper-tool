...
### FR-6 로컬MCP 관리 (별도 창)
> **모델 변경(2026-06)**: 프리셋 3분할 입력·스코프 라디오·직접입력 모달을 폐기하고 **통문장 단일 입력 실행** 모델로 단순화. 아래 FR-6.x는 갱신 모델 기준.

- 목적: 선택한 프로젝트(들)에서 **통문장 명령**(`claude mcp add ...` 전체)을 실행해 MCP 서버를 추가하고, local 스코프 설치분을 추적·제거한다.
- 진입: 메인 창에 "로컬MCP 관리" 버튼 → **별도 "로컬MCP 관리" 창** 오픈. 메인 런처는 단순하게 유지.
- 설치 메커니즘: **`claude mcp add` CLI**로만 등록한다(직접 JSON 작성 금지 — 사용자 글로벌 룰). 사용자가 입력한 통문장을 원본 그대로 각 선택 프로젝트를 작업 디렉터리로 실행.
  - 추가: 사용자 통문장 그대로 (예: `claude mcp add -s local <name> -- <command> [args...]`)
  - 제거: `claude mcp remove -s <scope> <name>` (**항상 스코프 명시** → 글로벌 보호)

- **FR-6.1 스코프**: 별도 라디오 없음 — 스코프는 통문장(`-s`/`--scope`)에 포함. 추적·제거 **관리 대상은 local만**(user/project는 실행만 하고 추적 제외).
- **FR-6.2 MCP 정의 출처**: 단일 명령 입력 필드. 직접 타이핑(단발성) 또는 프리셋 선택(클릭 시 입력 필드에 채워짐) — 둘의 실행 경로 동일.
- **FR-6.3 프리셋 관리(CRUD)**: 프리셋 저장/삭제. 별도 파일 `mcp_presets.json`에 저장.
  - 프리셋 스키마: `{ Name, CommandLine }` (통문장 통째 저장). 구버전 `{Name,Command,Args}`는 마이그레이션하지 않음(스킵).
- **FR-6.4 다중 프로젝트 일괄 적용**: 프로젝트 목록(런처의 `PathHistory` 재사용)에서 **다중 선택** → 선택 프로젝트들에 동일 통문장을 반복 실행. (프리셋은 단일 선택)
...
| 로컬MCP 관리 UI | 별도 창 | 메인에 "로컬MCP 관리" 버튼 → 별도 창. 단일 명령 입력 필드 + 실행(메인 "경로 실행" 패턴) |
| MCP 등록 방식 | 통문장 `cmd /c` 실행 | `claude mcp add` 통문장 원본 그대로 실행(가공 금지). 제거는 스코프 명시로 글로벌 보호 |
| 프리셋 저장 | 별도 `mcp_presets.json` | config와 분리. `{Name, CommandLine}` 통문장 저장 |
...
- FR-1~FR-7: 구현 완료. (FR-6 로컬MCP 관리는 통문장 단일 입력 실행 모델로 갱신·구현 완료 — 미해결 없음)
